...
public class ClientSQLResource implements ApiMarker {
  ...
  public static final String CLEANUP_TEMP_CHG =
      "DELETE FROM TMP_LC_TRK_CHG WHERE RUN_ID = ?";
  //@formatter:on
  ...
  public static final String SEL_REPL_TIME_REAL =
        "SELECT \n"
      + "    X.TBL\n"
      + "  , NVL(X.REP_AVG,0) AS rep_avg\n"
      + "  , NVL(X.REP_MIN,0) AS rep_min\n"
      + "  , NVL(X.REP_MAX,0) AS rep_max\n"
      + "FROM (\n"
      + " SELECT \n"
      + "    'ADDRS_T'                          AS tbl\n"
      + "   , AVG(ADDED_TS - IBMSNAP_LOGMARKER) AS rep_avg\n"
  ...
  //@formatter:off
  public static final String SEL_REPL_TIME_TOO_PESSIMISTIC=
        "SELECT x.* FROM (\n"
   ...
   */
  //@formatter:off
  public static final String SEL_REPL_TIME_IBM_WRONG =
        "WITH STEP1 AS (\n"
      + " SELECT\n"
      + "    m.SOURCE_TABLE,\n"
      + "    t.LASTRUN,\n"
